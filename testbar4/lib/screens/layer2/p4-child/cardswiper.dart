...
class CardswiperCPState extends State<CardswiperCP> {
          ...
          return SizedBox(
            ...
            child: CardSwiper(
              ...
              initialIndex: selectedIndex >= 0
                  ? selectedIndex
                  : 0, // ถ้า selectedIndex มีค่ามากกว่า 0 ก็จะเริ่มต้นด้วย index นั้น
              cardBuilder:
                  (context, index, percentThresholdX, percentThresholdY) {
                final challenge = challenges[index];
                final challengeId = challenge['challengeId'];
                final status =
                    widget.challengeStatuses[challengeId] ?? 'Unknown';
                ...
                */
                  ...
                  onDoubleTap: () {
                    final challenge = challenges[index];
                    final status =
                        widget.challengeStatuses[challenge['challengeId']] ??
                            'Unknown';
                    ...
                    if (status == 'in progress') {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                              'You are already in progress with this challenge!'),
                      ...
                      setState(() {
                        if (pickChallenge == index) {
                          pickChallenge =
                              -1; // ยกเลิกการเลือกถ้ากดซ้ำการ์ดที่เลือกแล้ว
                        } else {
                          pickChallenge = index; // select new card
                          selectedIndex = index; // update selectIndex
                        }
                  ...
                  child: _buildChallengeCard(
                      challenge, status, index == pickChallenge),
                );
              },
  ...
  Widget _buildChallengeCard(
      Map<String, dynamic> challenge, String status, bool isSelected) {
    Color statusColor;
