...
class UserChallenge{
  ...
  //add UserChallenge
  static Future<void> startChallenge(String userId, String challengeId, Timestamp startDate, Timestamp endDate) async {
  ...
  //Checking challenge
  static Future<void> checkChallengeCompletion(String userId, String challengeId) async {
  // รับรายละเอียดของความท้าทาย
  DocumentSnapshot challengeDoc = await firestore.collection('Challenges').doc(challengeId).get();
  ...
  double totalDistance =  await Activity.fetchTotalDistanceSE(startDate,endDate) ;
  // อัปเดตสถานะของความท้าทาย
  if (totalDistance >= requiredDistance) {
    await FirebaseFirestore.instance.collection('UserChallenges')
