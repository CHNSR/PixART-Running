...
import 'package:testbar4/screens/guest/login/child/ThirdpatySigin.dart';
import 'package:testbar4/services/auth_service/fire_auth.dart';
...
class _AddDataForThirdpatyState extends State<AddDataForThirdpaty> {
  ...
  User? _user;
  //final //ThirdPartySignIn _//ThirdPartySignIn = //ThirdPartySignIn();
  ...
  //   User? user = await _//ThirdPartySignIn.loginWithFacebook();
  //   if (user != null) {
  //     setState(() {
  //       _user = user;
  ...
  Future<void> _updateUserInfo() async {
    ...
    double weight =
        double.tryParse(weightController.text) ?? 0; // กำหนดค่าดีฟอลต์เป็น 0
    double height =
        double.tryParse(heightController.text) ?? 0; // กำหนดค่าดีฟอลต์เป็น 0
    DateTime birthday = DateTime.tryParse(birthdayController.text) ??
        DateTime.now(); // กำหนดค่าดีฟอลต์เป็นวันนี้
    int weeklyGoal =
        int.tryParse(weeklyGoalController.text) ?? 0; // กำหนดค่าดีฟอลต์เป็น 0
        ...
        child: Center(
          child: Column(
            children: [
              ...
              Image.asset(iconPath.appBarIcon('register_outline'),
                  height: 200, width: 200),
              const SizedBox(height: 40),
              const Text('Register to PixART Running Tracking',
                  style: TextStyle(
                      fontSize: 20,
